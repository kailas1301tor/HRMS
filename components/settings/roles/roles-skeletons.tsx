...
export const RoleCardSkeleton = () => (
  <div className="bg-midnight border border-border/60 rounded-xl p-4 flex flex-col justify-between min-h-[120px]">
    <div className="space-y-3">
      <Skeleton className={cn('h-4 w-1/3 rounded-xl', uiSkeletonBlock)} />
      <Skeleton className={cn('h-3 w-3/4 rounded-xl', uiSkeletonBlock)} />
      <Skeleton className={cn('h-3 w-2/3 rounded-xl', uiSkeletonBlock)} />
    </div>
  </div>
)
    ...
    {Array.from({ length: 6 }).map((_, i) => (
      <div key={i} className="flex items-center justify-between p-3.5 rounded-xl border border-border/40 bg-midnight">
        <div className="space-y-2 flex-1">
          <Skeleton className={cn('h-3 w-1/2 rounded-xl', uiSkeletonBlock)} />
          <Skeleton className={cn('h-2.5 w-1/4 rounded-xl', uiSkeletonBlock)} />
        </div>
  ...
  <Card className="bg-card/35 backdrop-blur border border-border/80 shadow-lg">
    <CardHeader className="pb-4 border-b border-border/40">
      <div className="flex items-center gap-4">
        <Skeleton className={cn('h-9 w-20 rounded-xl', uiSkeletonBlock)} />
        <div className="space-y-2 flex-1">
          <Skeleton className={cn('h-5 w-1/4 rounded-xl', uiSkeletonBlock)} />
          <Skeleton className={cn('h-3 w-1/3 rounded-xl', uiSkeletonBlock)} />
        </div>
      </div>
    </CardHeader>
    <CardContent className="pt-6 space-y-6">
      <div className="space-y-2">
        <Skeleton className={cn('h-3 w-16 rounded-xl', uiSkeletonBlock)} />
        <Skeleton className={cn('h-10 w-full rounded-xl', uiSkeletonBlock)} />
      </div>
      <div className="space-y-3">
        <Skeleton className={cn('h-3 w-32 rounded-xl', uiSkeletonBlock)} />
        <Skeleton className={cn('h-10 w-full rounded-xl', uiSkeletonBlock)} />

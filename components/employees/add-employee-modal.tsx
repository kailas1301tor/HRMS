...
export function AddEmployeeModal({ open, onOpenChange, onSuccess, editEmployee }: AddEmployeeModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={cn(uiDialog, 'max-w-xl max-h-[90vh] overflow-y-auto')}>
        <DialogHeader>
            ...
            >
              ...
              <DialogFooter className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-2 pt-4 border-t border-border/40">
                ...
                <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto">
                  <DialogClose asChild>
                    <Button type="button" variant="outline" className={cn(uiOutlineBtn, 'text-xs')} disabled={isLoading}>
                      Cancel
                    </Button>
                  </DialogClose>
                  {addStep === 4 ? (
                    <PrimaryButton type="submit" isLoading={isLoading} className="text-xs">
                      {isEditMode ? 'Update Details' : 'Add Employee'}
                    </PrimaryButton>
                  ) : (
                    <PrimaryButton type="button" onClick={handleNextStep} className="text-xs">
                      Next
                    </PrimaryButton>
                  )}
                </div>

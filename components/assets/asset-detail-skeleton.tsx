  ...
  return (
    <div className="space-y-6" aria-label="Loading asset details" role="status">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <Skeleton className={cn('h-9 w-28 rounded-[20px] [corner-shape:squircle]', uiSkeletonBlock)} />
        <div className="flex items-center gap-2 self-end sm:self-auto">
          <Skeleton className={cn('h-10 w-24 rounded-[20px] [corner-shape:squircle]', uiSkeletonBlock)} />
          <Skeleton className={cn('h-10 w-10 rounded-[20px] [corner-shape:squircle]', uiSkeletonBlock)} />
        </div>
      </div>
      ...
      <div className={cn(uiCard, 'p-6 shadow-xl relative overflow-hidden')}>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            <Skeleton className={cn('w-14 h-14 rounded-[32px] [corner-shape:squircle]', uiSkeletonBlock)} />
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                <Skeleton className={cn('h-6 w-40 rounded-[20px] [corner-shape:squircle]', uiSkeletonBlock)} />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 w-full md:w-auto border-t md:border-t-0 border-border/50 pt-4 md:pt-0">
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="space-y-1.5">
                ...
                <Skeleton className={cn('h-5 w-24 rounded-[20px] [corner-shape:squircle]', uiSkeletonBlock)} />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="min-h-[250px] p-6 bg-card/20 border border-border/40 rounded-[32px] [corner-shape:squircle] space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {Array.from({ length: 2 }).map((_, col) => (
            <div key={col} className="space-y-4">
              <Skeleton className={cn('h-5 w-32 rounded-[20px] [corner-shape:squircle]', uiSkeletonBlock)} />
